<!-- <template>
  <div class="container mt-4">
    <div class="rounded shadow-sm mb-4 px-3 py-2 d-flex justify-content-between align-items-center" style="background:#d6f5d6;">
      <div>
        <span class="fw-bold" style="color:#d7263d;">Welcome to Admin</span>
        <span class="ms-4">
          <router-link to="/admin" class="nav-link d-inline px-2">Home</router-link>|
          <router-link to="/admin/users" class="nav-link d-inline px-2">Users</router-link>|
          <router-link to="/admin/search" class="nav-link d-inline px-2">Search</router-link>|
          <router-link to="/admin/summary" class="nav-link d-inline px-2">Summary</router-link>|
          <a href="#" class="nav-link d-inline px-2" @click.prevent="logout">Logout</a>
        </span>
      </div>
      <div>
        <router-link to="/admin/edit-profile" class="nav-link d-inline" style="color:#1e90ff;">Edit Profile</router-link>
        <span class="ms-2" style="color:orange;">View/Edit Profile details</span>
      </div>
    </div>

    <h3 class="text-center mb-4" style="color:#3366cc;">Parking Lots</h3>
        ...
        <div class="card shadow-sm">
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <div>
                <router-link :to="`/admin/edit-lot/${lot.id}`" class="btn btn-sm btn-warning me-2">Edit</router-link>
                <button class="btn btn-sm btn-danger" @click="deleteLot(lot.id)">Delete</button>
              </div>
            </div>
            <div class="mb-2 text-muted" style="font-size: 0.95em;">
              {{ lot.address }} | Pin: {{ lot.pin_code }} | ₹{{ lot.price }}/hr
            </div>
            <div class="mb-2">
              <span class="text-success" style="font-size:1.1em;">(Occupied : {{ lot.occupied_spots }}/{{ lot.maximum_number_of_spots }})</span>
            </div>
            <div class="d-flex flex-wrap border rounded p-2" style="background:#f7f7f7;">
              <span
                v-for="spot in lot.spots"
                :key="spot.id"
                class="border rounded m-1 d-flex justify-content-center align-items-center"
                :style="{
                  width: '32px', height: '32px',
                  background: spot.status === 'O' ? '#ffb3b3' : '#b3ffd9',
                  color: '#333', fontWeight: 'bold',
                  cursor: spot.status === 'O' ? 'pointer' : 'default'
                }"
    ...
    <div class="text-center mt-4">
      <router-link to="/admin/new-lot" class="btn" style="background:#99ccff; color:#1a1a1a; border:2px solid orange; font-weight:500; font-size:1.2em;">
        + Add Lot
      </router-link>
    </div>
...
const deleteLot = async (id) => {
  ...
  try {
    ...
    const data = await res.json()
    if (!res.ok) {
      error.value = data.msg || 'Delete failed'
    } else {
      fetchLots()
    }
  } catch (e) {
